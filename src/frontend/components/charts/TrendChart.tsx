...
export function TrendChart({ data, targetDesk, targetGate }: Props) {
  // Default to gate per user preference.
  const [metric, setMetric] = useState<Metric>("gate");
  const target = metric === "gate" ? targetGate : targetDesk;
  const valueOf = (r: EntryWithDur) => (metric === "gate" ? r.gate : r.desk);
  ...
  const pts = rows.map((r, i) => ({ x: x(i), y: y(valueOf(r)), r, v: valueOf(r) }));
    ...
    <>
      ...
      <div ref={ref} style={{ width: "100%", position: "relative" }}>
          ...
          >
            ...
            {pts.map((p, i) => (
              <circle
                ...
                cy={p.y}
                r={hover === i ? 5 : 2.8}
                fill={p.v > target ? "var(--late)" : "var(--accent)"}
                stroke="var(--surface)"
        ...
        {hover != null && w > 0 && (
          ...
          >
            {shortDate(rows[hover].date)} · {metric} {fmt(pts[hover].v)}
          </div>
        )}
      </div>
    </>
